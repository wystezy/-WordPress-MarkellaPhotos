<?php 
add_action( 'wp_enqueue_scripts', 'notes_and_photos_enqueue_styles' );
function notes_and_photos_enqueue_styles() {
	wp_enqueue_style( 'notes-and-photos-parent-style', get_template_directory_uri() . '/style.css' ); 
} 

function notes_and_photos_customize_preview_js() {
	wp_enqueue_script( 'notes-and-photos-customizer', get_stylesheet_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '1', true );
}
add_action( 'customize_preview_init', 'notes_and_photos_customize_preview_js' );

function notes_and_photos_google_fonts() {
	wp_enqueue_style( 'notes-and-photos-google-fonts', '//fonts.googleapis.com/css2?family=Noto+Sans+TC:wght@400;500;700&display=swap', false ); 
}
add_action( 'wp_enqueue_scripts', 'notes_and_photos_google_fonts' );


require_once( get_stylesheet_directory() . '/inc/custom-header.php' );


function notes_and_photos_customize_register( $wp_customize ) {	
	$wp_customize->add_section( 'post_feed_button', array(
		'title'      => __('Post feed button','blogrid'),
		'priority'   => 1,
		'capability' => 'edit_theme_options',
	) );
	$wp_customize->add_setting( 'post_feed_button_border_color', array(
		'default'           => '#fab526',
		'sanitize_callback' => 'sanitize_hex_color',
		'transport'         => 'postMessage',
	) );
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'post_feed_button_border_color', array(
		'label'       => __( 'Post feed button border color', 'blogrid' ),
		'section'     => 'post_feed_button',
		'priority'   => 1,
		'settings'    => 'post_feed_button_border_color',
	) ) );
	$wp_customize->add_setting( 'post_feed_button_text_color', array(
		'default'           => '#000',
		'sanitize_callback' => 'sanitize_hex_color',
		'transport'         => 'postMessage',
	) );
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'post_feed_button_text_color', array(
		'label'       => __( 'Post feed button text color', 'blogrid' ),
		'section'     => 'post_feed_button',
		'priority'   => 1,
		'settings'    => 'post_feed_button_text_color',
	) ) );
...
add_action( 'customize_register', 'notes_and_photos_customize_register', 999 );




if(! function_exists('notes_and_photos_customize_register_output' ) ):
	function notes_and_photos_customize_register_output(){
		?>

		<style type="text/css">
			a.readmore-btn {border-color:<?php echo esc_attr(get_theme_mod( 'post_feed_button_border_color')); ?>; }
			a.readmore-btn {color:<?php echo esc_attr(get_theme_mod( 'post_feed_button_text_color')); ?>; }
			.content-wrapper .post-data-text .about-the-author-byline, .single .post-data-text .about-the-author-byline{ color: <?php echo esc_attr(get_theme_mod( 'notes_and_photos_abouttheauthor_color')); ?>; }
			a.read-more-btn{ background: <?php echo esc_attr(get_theme_mod( 'notes_and_photos_readmore_bg')); ?>; }
			a.read-more-btn{ color: <?php echo esc_attr(get_theme_mod( 'notes_and_photos_readmore_text')); ?>; }
	...
	<?php }
	add_action( 'wp_head', 'notes_and_photos_customize_register_output' );
endif;
...
add_action( 'tgmpa_register', 'notes_and_photos_register_required_plugins' );
function notes_and_photos_register_required_plugins() {
	$plugins = array(
	...
	$config = array(
		'id'           => 'notes-and-photos',                 // Unique ID for hashing notices for multiple instances of TGMPA.
